...
from app.schemas.log_cuti import LogCutiCreate, LogCutiOut, RiwayatCutiOut, PengajuanOngoingOut
from app.services.leave_service import create_leave_request, get_my_leaves, get_my_ongoing_leaves
...
async def get_my_ongoing_leave_list(
    current_user: Annotated[User, Depends(require_role("karyawan", "hr", "pm"))],
    db: Annotated[AsyncSession, Depends(get_db)],
):
    return await get_my_ongoing_leaves(current_user.id_user, db)